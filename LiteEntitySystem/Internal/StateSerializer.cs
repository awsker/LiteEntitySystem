...
using static LiteEntitySystem.ServerEntityManager;
...
namespace LiteEntitySystem.Internal
{
    internal struct StateSerializer
    {
        ...
        public unsafe bool MakeDiff(byte playerId, ushort serverTick, ushort minimalTick, ushort playerTick, byte* resultData, ref int position, HumanControllerLogic playerController, ServerEntityManager.ChangeType changeType)
        {
            if (_entity == null)
            {
            ...
            if (changeType == ChangeType.FullUpdate ||
                playerController != null && playerController.IsEntityNeedForceSync(_entity, playerTick) ||
                ...
                fixed (byte* lastEntityData = _latestEntityData) //make diff
                {
                    ...
                    *fieldFlagAndSize = 0;
                    ...
                    for (int i = 0; i < _fieldsCount; i++)
                    {
                        ...
                        if (!mustSendDestroyedField && ((field.Flags & SyncFlags.OnlyForOwner) != 0 && !isOwned ||
